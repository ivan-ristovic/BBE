	...
	{	
	    ...
	    while (it.hasNext()) {		    
			...
			if (srcValue != destValue) {
				System.out.println("error: " + pair.getKey() + "(" + srcValue + ") != " + name + "(" + destValue + ")");
				conflictingVars.add(pair.getKey());
				hasBlockConflicts = true;
			}
	    }
	    System.out.println("[INFO] Block (" + currentDepth + ") traversed with conflicts: " + hasBlockConflicts);
	    if (hasBlockConflicts)
	    {
		    System.out.println("[INFO] Conflicting vars: " + conflictingVars.toString());
		    System.out.println("[INFO] Deep diving into into block ( " + currentDepth + ")");
